...
const demoMembers = [
  ...
  { id: 4, uid: "member_1001", name: "이고객", role: "funeral_member", parentUid: "sales_001", status: "joined" },
  { id: 5, uid: "member_1002", name: "박고객", role: "funeral_member", parentUid: "admin", status: "pending" },
];
...
router.get("/", async (req, res, next) => {
  try {
    ...
    const result = await pool.request().query(`
      SELECT id, uid, username, name, phone, email, role, parent_uid AS parentUid, status
...
router.get("/tree", async (req, res, next) => {
  try {
    ...
    const result = await pool
      ...
      .query(`
        WITH UserTree AS (
          SELECT id, uid, username, name, phone, email, role, parent_uid AS parentUid, status, 0 AS depth
          FROM dbo.MaruPartnerUsers
          WHERE uid = @rootUid

          UNION ALL

          SELECT child.id, child.uid, child.username, child.name, child.phone, child.email,
                 child.role, child.parent_uid AS parentUid, child.status, parent.depth + 1
          FROM dbo.MaruPartnerUsers child
          INNER JOIN UserTree parent ON child.parent_uid = parent.uid
        )
        SELECT id, uid, username, name, phone, email, role, parentUid, status, depth
        FROM UserTree
